...
### CONFIG OF BOT ###
#####################
define('DEBUG_FILE_NAME', 'imbot.log'); // if you need read debug log, you should write unique log name
...
if ($_REQUEST['event'] == 'ONIMBOTMESSAGEADD')
{
	...
	{
        if ($_REQUEST['data']['PARAMS']['MESSAGE'] === 'add')
        {

            $loc = print_r($result = restCommand('imbot.app.register', Array(

                'BOT_ID' => 6, // Идентификатор бота владельца приложения для чата
                'CODE' => 'space2', // Ккод приложения для чата
                'IFRAME' => 'https://bitrix-test.melalex.ru/botEcho/iframe.php',
                'IFRAME_WIDTH' => '250', // Желаемая ширина фрейма. Минимальное значение - 250px
                'IFRAME_HEIGHT' => '350', // Желаемая высота фрейма. Минимальное значение - 50px
                'HASH' => 'd1ab17949a572b0979d8db0d5b349cd2', // Токен для доступа к вашему фрейму для проверки подписи, 32 символа.
                'ICON_FILE' => base64_encode(file_get_contents(__DIR__.'/icon.png')), // Иконка вашего приложения - base64
                ...
                'LANG' => Array( // Массив переводов, желательно указывать как минимум для RU и EN
                    Array('LANGUAGE_ID' => 'ru', 'TITLE' => 'Echobot IFRAME', 'DESCRIPTION' => 'Open Echobot IFRAME app', 'COPYRIGHT' => 'Bitrix24'),
                )
            ...
            $result = restCommand('imbot.message.add', Array(
                ...
                "ATTACH" => Array(
                    Array("MESSAGE" => "reply: $loc"),
                    Array("MESSAGE" => "latency: ".$latency)
                )
            ), $_REQUEST["auth"]);
        }
        elseif ($_REQUEST['data']['PARAMS']['MESSAGE'] === 'update')
        {
            $loc = print_r($result = restCommand('imbot.app.update', Array(

                'APP_ID' => 8, // Идентификатор чата
                'FIELDS' => Array(
                    'IFRAME' => 'https://bitrix-test.melalex.ru/botEcho/iframe.php',
                    'IFRAME_WIDTH' => '250', // Желаемая ширина фрейма. Минимальное значение - 250px
                    'IFRAME_HEIGHT' => '350', // Желаемая высота фрейма. Минимальное значение - 50px
                    'HASH' => 'd1ab17949a572b0979d8db0d5b349cd2', // Токен для доступа к вашему фрейму, 32 символа.
                    'ICON_FILE' => base64_encode(file_get_contents(__DIR__.'/icon.png')), // Иконка вашего приложения - base64
                    'CONTEXT' => 'ALL', // Контекст приложения
                    'EXTRANET_SUPPORT' => 'N', // Доступна ли команда пользователям экстранет, по умолчанию N
                    'LIVECHAT_SUPPORT' => 'N', // Поддержка онлайн-чата
                    'IFRAME_POPUP' => 'N', // iframe будет открыт с возможностью перемещения внутри мессенджера, переход между диалогами не будет закрывать такое окно
                    'LANG' => Array( // Массив переводов, желательно указывать как минимум для RU и EN
                        Array('LANGUAGE_ID' => 'ru', 'TITLE' => 'Смайлики от СТЕК', 'DESCRIPTION' => 'Набор смайликов от компании "СТЕК"', 'COPYRIGHT' => 'STEK'),
                    )
                )
            ), $_REQUEST["auth"]), 1);
        ...
        elseif ($_REQUEST['data']['PARAMS']['MESSAGE'] === 'deleteapp')
        {
            $loc = print_r($result = restCommand('imbot.app.unregister', Array(

                'APP_ID' => 12, // Идентификатор команды для удаления
